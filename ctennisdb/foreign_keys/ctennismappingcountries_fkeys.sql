...
begin
   ...
   __foreign_key_name :='ctennismappingcountries_ctcountries_ctcodeofficial_fkey';
   ...
   if not exists (select 1 from information_schema.table_constraints where table_schema='public' and table_name=__table_name and constraint_name=__foreign_key_name) then
      ...
      execute 'alter table ' || __table_name || ' add constraint ' || __foreign_key_name || ' foreign key (ctcodeofficial) references ctenniscountries(ctcode);'; 
