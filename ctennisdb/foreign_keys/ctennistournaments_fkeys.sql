...
begin
   ...
   __foreign_key_name :='ctennistournaments_ctcountries_ctcode_fkey';
   ...
   __foreign_key_name :='ctennistournaments_cttourn_cat_ctcode_fkey';

   if not exists (select 1 from information_schema.table_constraints where table_schema='public' and table_name=__table_name and constraint_name=__foreign_key_name) then

      raise notice 'The foreign key <%> does not exist. It will be created.',__foreign_key_name;

      execute 'alter table ' || __table_name || ' add constraint ' || __foreign_key_name || ' foreign key (ctcategory) references ctennistournamentscategories(ctdid);';  
   ...
   __foreign_key_name :='ctennistournaments_ctsurface_ctcode_fkey';

   if not exists (select 1 from information_schema.table_constraints where table_schema='public' and table_name=__table_name and constraint_name=__foreign_key_name) then

      raise notice 'The foreign key <%> does not exist. It will be created.',__foreign_key_name;

      execute 'alter table ' || __table_name || ' add constraint ' || __foreign_key_name || ' foreign key (ctsurface) references ctennissurfaces(ctcode);';  
