...
begin
   ...
   if not exists (select 1 from information_schema.tables where table_schema='public' and table_name=__table_name and table_type='BASE TABLE') then
     ...
     create table cttournamentrounds
     	(
     		ctcode serial,
        ctcodeofficial varchar(100) not null constraint uniq_cttournamentrounds_ctcodeofficial unique,
     		ctlabel varchar(100) not null,
